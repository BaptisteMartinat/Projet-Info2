...
int main() {
    ...
    while (!key[KEY_ESC]) {
        ...
        if (saisir_pseudo) {
            ...
            const char *label = "NOUVELLE PARTIE";
            ...
            textout_centre_ex(buffer, font, label, SCREEN_W / 2, by + 15, makecol(0, 0, 0), -1);

            if (mouse_b & 1 && mouse_x >= bx && mouse_x <= bx + bw && mouse_y >= by && mouse_y <= by + bh) {
                rest(200);
