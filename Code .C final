...
int detecter_arrivee(EtatJeu *etat) {
    ...
    if (etat->niveau_actuel == NIVEAU_MER) {
        int fin_x = FIN_MER_X - etat->decor_scroll;
        int fin_y = FIN_MER_Y;
        int fin_w = etat->img_fin_mer ? etat->img_fin_mer->w : 100;
        int fin_h = etat->img_fin_mer ? etat->img_fin_mer->h : 100;

        if (collision(etat->joueur.x, etat->joueur.y,
                     etat->joueur.largeur, etat->joueur.hauteur,
                     fin_x, fin_y,
                     fin_w, fin_h)) {
            return 1;  // Fin du niveau détectée
                     }
    }
...
void gestion_collision_roues(EtatJeu *etat) {
    ...
    for (int i = 0; i < NB_ROUES; i++) {
        ...
        if (distance2 < rayon_collision * rayon_collision) {
            etat->joueur_est_mort = 1; // Toujours déclencher la mort
            return;
        }
    }
}
...
void afficher_scene(EtatJeu *etat) {
    ...
    for (int i = 0; i < NB_POISSON; i++) {
        int x = etat->position_poisson[i][0] - etat->decor_scroll;
        int y = etat->position_poisson[i][1];
        draw_sprite(etat->page, etat->img_poisson, x, y);

        // Debug hitbox - nouvelle version
        const int poisson_w = 100;
        const int poisson_h = 120;
        int hitbox_x = x + (138 - poisson_w)/2;
        int hitbox_y = y + (198 - poisson_h)/2;
        rect(etat->page, hitbox_x, hitbox_y,
             hitbox_x + poisson_w, hitbox_y + poisson_h,
             makecol(255, 0, 0));
    }

    //affichage crabes
    for (int i = 0; i < NB_CRABE; i++) {
        int x = etat->position_crabe[i][0] - etat->decor_scroll;
        int y = etat->position_crabe[i][1];
        draw_sprite(etat->page, etat->img_crabe, x, y);

        // Debug hitbox - nouvelle version
        const int crabe_w = 100;
        const int crabe_h = 120;
        int hitbox_x = x + (138 - crabe_w)/2;
        int hitbox_y = y + (198 - crabe_h)/2;
        rect(etat->page, hitbox_x, hitbox_y,
             hitbox_x + crabe_w, hitbox_y + crabe_h,
             makecol(255, 0, 0));
    }
    ...
    if (etat->niveau_actuel == NIVEAU_MER) {
        ...
        if (fin_x > -200 && fin_x < MY_SCREEN_W) {
            if (etat->img_fin_mer) {
                draw_sprite(etat->page, etat->img_fin_mer, fin_x, fin_y);
            } else {
                // Fallback: rectangle rouge si l'image n'est pas chargée
                rectfill(etat->page, fin_x, fin_y,
                        fin_x + 100, fin_y + 100,
                        makecol(255, 0, 0));
            }
        }
...
void nettoyer_ressources(EtatJeu *etat) {
    ...
    //poisons
    if (etat->img_poisson) {
        destroy_bitmap(etat->img_poisson);
        etat->img_poisson = NULL;
    }
//crabes
    if (etat->img_crabe) {
        destroy_bitmap(etat->img_crabe);
        etat->img_crabe = NULL;
    }
...
void initialiser_ressources_niveaux(NiveauRessources *niveaux) {
    ...
    niveaux[NIVEAU_VOLCAN] = (NiveauRessources){
        ...
        .poisson_path = "poisson.bmp",
        .crabe_path = "crabe.bmp",
        ...
        .positions_poisson = {{20000, 0}, {20000, 0}, {20000, 0}, {20000, 0}},
        .positions_crabe = {{20000, 0}, {20000, 0}, {20000, 0},{20000, 0},{20000, 0},{20000, 0}, {20000, 0}},
    ...
    niveaux[NIVEAU_MER] = (NiveauRessources){
        ...
        .positions_champi = {{20000, 0}, {20000, 0}, {20000, 0}, {20000, 0}}, // Positions par défaut
        .positions_liane = {{20000, 0}, {20000, 0}, {20000, 0}, {20000, 0}}, // Positions par défaut
        .positions_poisson = {{760, 230}, {3250, 300}, {5800, 100}, {7500, 270}},
        .positions_crabe = {{1220, 270}, {3450, 165}, {3750, 325}, {4250, 165}, {4850, 160}, {4850, 410}, {8100, 135}},
        .musique_path = "mer.wav",
        .explosion_path = "explosion_mer.bmp",
        .trainee_path = "saut3.bmp",
        .img_fin_path = "fin_mer.bmp",
        .img_gros_path = "gros.bmp",
        .img_petit_path = "petit.bmp",
        .checkpoint_path = "checkpoint3.bmp",
        .roues_paths = {"roue0.bmp", "roue1.bmp", "roue2.bmp", "roue3.bmp"},
        .positions_cp = {{600, 450}, {2400, 470}, {5300, 470}, {7300, 470}},
        .roues_positions = {{20000, 170,0},
        {20000, 390,0},
        {20000, 130,0},
        {20000, 420,0},{20000, 420,0}}
...
void gestion_collision_crabe(EtatJeu *etat) {
    ...
    for (int i = 0; i < NB_CRABE; i++) {
        // Ignorer les crabes désactivés (position x = 20000)
        if (etat->crabes[i].x == 20000) continue;

        // Dimensions de la hitbox du crabe (ajustées légèrement)
        const int crabe_w = 60;   // Largeur de la hitbox
        const int crabe_h = 45;   // Hauteur de la hitbox (légèrement augmentée)

        // Position du crabe avec décalage pour centrer la hitbox plus bas
        int crabe_x = etat->crabes[i].x + 20;  // Décalage horizontal
        int crabe_y = etat->crabes[i].y + 15;  // Décalage vertical légèrement augmenté
        ...
        if (collision(joueur_x, joueur_y,
            ...
            if (etat->son_explosion) {
                play_sample(etat->son_explosion, 255, 128, 1000, 0);
            }

            return; // On sort après la première collision détectée
                     }
    }
}
...
void gestion_collision_poisson(EtatJeu *etat) {
    ...
    for (int i = 0; i < NB_POISSON; i++) {
        ...
        const int poissons_w = 60;   // Largeur de la hitbox
        const int poissons_h = 50;   // Hauteur de la hitbox (augmentée)

        // Position du crabe avec décalage pour centrer la hitbox plus bas
        int poissons_x = etat->poissons[i].x + 20;  // Décalage horizontal
        int poissons_y = etat->poissons[i].y + 25;  // Décalage vertical augmenté

        // Vérification de collision
        if (collision(joueur_x, joueur_y,
                     etat->joueur.largeur, etat->joueur.hauteur,
                     poissons_x, poissons_y,
                     poissons_w, poissons_h)) {
...
void jeu_scrolling(const char *pseudo, TypeNiveau niveau, SAMPLE *lave, SAMPLE *menu, SAMPLE* montagne, SAMPLE* mer) {
    ...
    switch (niveau) {
        case NIVEAU_MER:
            ...
            // Gros bonus (3 instances)
                etat.bonus_pos[0][0][0] = 1600; etat.bonus_pos[0][0][1] = 270;
        etat.bonus_pos[0][1][0] = 4500; etat.bonus_pos[0][1][1] = 250;
        etat.bonus_pos[0][2][0] = 8700; etat.bonus_pos[0][2][1] = 270;

        // Petit bonus (3 instances)
        etat.bonus_pos[1][0][0] = 300; etat.bonus_pos[1][0][1] = 200;
        etat.bonus_pos[1][1][0] = 4000; etat.bonus_pos[1][1][1] = 180;
        etat.bonus_pos[1][2][0] =7750; etat.bonus_pos[1][2][1] = 300;
        ...
        etat.bonus_pos[2][2][0] = 6000; etat.bonus_pos[2][2][1] = 300;

        etat.bonus_pos[3][0][0] = 20000; etat.bonus_pos[3][0][1] = 350;
        etat.bonus_pos[3][1][0] = 15200; etat.bonus_pos[3][1][1] = 300;
        etat.bonus_pos[3][2][0] = 70200; etat.bonus_pos[3][2][1] = 300;
        break;
        ...
        etat.bonus_pos[3][0][0] = 4000; etat.bonus_pos[3][0][1] = 350;
        etat.bonus_pos[3][1][0] = 1500; etat.bonus_pos[3][1][1] = 300;
        etat.bonus_pos[3][2][0] = 7000; etat.bonus_pos[3][2][1] = 300;
            break;

        case NIVEAU_VOLCAN:
            musique_niveau = load_sample("lave.wav");
            // Positions par défaut
                etat.bonus_pos[0][0][0] = 20000; etat.bonus_pos[0][0][1] = 300;
        etat.bonus_pos[0][1][0] = 20000; etat.bonus_pos[0][1][1] = 250;
        etat.bonus_pos[0][2][0] = 20000; etat.bonus_pos[0][2][1] = 350;

        // Petit bonus (3 instances)
        etat.bonus_pos[1][0][0] = 20000; etat.bonus_pos[1][0][1] = 200;
        etat.bonus_pos[1][1][0] = 20000; etat.bonus_pos[1][1][1] = 180;
        etat.bonus_pos[1][2][0] = 20000; etat.bonus_pos[1][2][1] = 220;

        // Vite bonus (3 instances)
        etat.bonus_pos[2][0][0] = 20000; etat.bonus_pos[2][0][1] = 350;
        etat.bonus_pos[2][1][0] = 20000; etat.bonus_pos[2][1][1] = 300;
        etat.bonus_pos[2][2][0] = 20000; etat.bonus_pos[2][2][1] = 280;

        etat.bonus_pos[3][0][0] = 20000; etat.bonus_pos[3][0][1] = 350;
        etat.bonus_pos[3][1][0] = 20000; etat.bonus_pos[3][1][1] = 300;
        etat.bonus_pos[3][2][0] = 20000; etat.bonus_pos[3][2][1] = 300;
            break;
    }
    ...
    while (!key[KEY_ESC]) {
        ...
        if (etat.joueur_est_mort == 1) {
            ...
            if ((clock() - temps_mort) >= 1000) {
                if (etat.dernier_cp != -1) {
                    etat.joueur_est_mort = 2; // Phase de respawn
                } else {
                    // Retour au menu après avoir affiché l'explosion
                    stop_sample(musique_niveau);
                    stop_sample(lave);
        ...
        gestion_collision_crabe(&etat);
        gestion_collision_poisson(&etat);
        ...
        // Détection de l'arrivée
         if (detecter_arrivee(&etat)) {
            if(niveau == NIVEAU_VOLCAN) {
                stop_sample(lave);
                play_sample(montagne, 200, 100, 920, 1);
                jeu_scrolling(pseudo, NIVEAU_MONTAGNE, lave, menu, montagne, mer);
            } else if (niveau == NIVEAU_MONTAGNE) {
                stop_sample(montagne);
                play_sample(mer, 200, 100, 920, 1);
                jeu_scrolling(pseudo, NIVEAU_MER, lave, menu, montagne, mer);
            }else {
                clear_bitmap(etat.page);
                textout_centre_ex(etat.page, font, "NIVEAU MER TERMINE!",
                                 MY_SCREEN_W/2, MY_SCREEN_H/2 - 20,
                                 makecol(255, 255, 255), -1);
                textout_centre_ex(etat.page, font, "TEMPS: ",
                                 MY_SCREEN_W/2, MY_SCREEN_H/2 + 10,
                                 makecol(255, 255, 255), -1);
                textprintf_centre_ex(etat.page, font, MY_SCREEN_W/2, MY_SCREEN_H/2 + 30,
                                   makecol(255, 255, 255), -1,
                                   "%d secondes", etat.temps_jeu);
                blit(etat.page, screen, 0, 0, 0, 0, MY_SCREEN_W, MY_SCREEN_H);
...
void charger_ressources_niveau(EtatJeu *etat) {
    ...
    etat->img_poisson = load_bitmap(n->poisson_path, NULL);
    etat->img_crabe = load_bitmap(n->crabe_path, NULL);

    if (!etat->img_poisson) {
        allegro_message("Erreur chargement %s", n->poisson_path);
        exit(1);
    }


    if (!etat->img_crabe) {
        allegro_message("Erreur chargement %s", n->crabe_path);
        exit(1);
    }
    ...
    // Initialiser les positions des champignons
    for (int i = 0; i < NB_POISSON; i++) {
        etat->poissons[i].x = n->positions_poisson[i][0];
        etat->poissons[i].y = n->positions_poisson[i][1];
        etat->position_poisson[i][0] = n->positions_poisson[i][0];
        etat->position_poisson[i][1] = n->positions_poisson[i][1];
    }

    // Initialiser les positions des crabes
    for (int i = 0; i < NB_CRABE; i++) {
        etat->crabes[i].x = n->positions_crabe[i][0];
        etat->crabes[i].y = n->positions_crabe[i][1];
        etat->position_crabe[i][0] = n->positions_crabe[i][0];
        etat->position_crabe[i][1] = n->positions_crabe[i][1];
    }
    ...
    // Charger l'image de fin pour le niveau mer
    etat->img_fin_mer = load_bitmap(n->img_fin_path, NULL);
    if (!etat->img_fin_mer) {
        allegro_message("Erreur chargement fin_mer.bmp");
        // On continue quand même avec un rectangle en fallback
    }
}
